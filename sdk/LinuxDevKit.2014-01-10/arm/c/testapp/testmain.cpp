...
PAR_ERROR startExposure(bool dualChannelMode, bool fastReadout, CCD_REQUEST ccd,
		double expTime, SHUTTER_COMMAND shutterCommand,
		READOUT_BINNING_MODE readoutMode, unsigned short top, unsigned short left,
...
PAR_ERROR startReadout(CCD_REQUEST ccd, READOUT_BINNING_MODE readoutMode,
		unsigned short top, unsigned short left, unsigned short height,
		unsigned short width);

PAR_ERROR endReadout(CCD_REQUEST ccd);

PAR_ERROR ccdReadout(CSBIGImg* pImg, bool dualChannelMode, CCD_REQUEST ccd,
		READOUT_BINNING_MODE readoutMode, unsigned short top, unsigned short left,
		unsigned short height, unsigned short width);
...
PAR_ERROR grabImageTest(int argc, char *argv[])
{
	...
	if (argc != 13)
	{
		cout << "Application startup error: Bad number of input parameters. "
				<< endl;
		cout
				<< "Format : ./testmain filePath fileType imgCount imgType expTime readoutMode top left width heigh fastReadout dualReadoutChannel"
				<< endl;
		cout
				<< "Example: ./testmain /observations/img/raw/ FITS 1 LF 0.01 1x1 0 0 0 0 1 1"
				<< endl;
		cout
				<< "Example: ./testmain /observations/img/raw/ SBIG 1 LF 0.01 1x1 0 0 0 0 0 0"
				<< endl;
	...
	cout << "Input parameters      : " << "filePath = " << filePath
			<< ", bFitsType = " << bFitsType << ", imgCount = " << numOfImages
			<< ", bLightFrame = " << bLightFrame << ", expTime = " << expTime
			<< ", readoutMode = " << rm << ", top = " << top << ", left = " << left
			<< ", width = " << width << ", height = " << height << ", bFastReadout = "
			<< bFastReadout << ", bDualChannelMode = " << bDualChannelMode << endl;
	...
	for (int i = 1; i <= numOfImages; i++)
	{
		do
		{
			if (bLightFrame)
			{
				...
				{
					cout << "CSBIGCam error        : " << pCam->GetErrorString(err)
							<< endl;
					break;
				}
			}
			...
			{
				filePathName += ".fits";
				if ((ferr = pImg->SaveImage(filePathName.c_str(), SBIF_FITS))
						!= SBFE_NO_ERROR)
				{
			...
			{
				filePath += ".sbig";
				if ((ferr = pImg->SaveImage(filePathName.c_str(), SBIF_COMPRESSED))
						!= SBFE_NO_ERROR)
				{
...
PAR_ERROR fpsTest(int argc, char *argv[])
{
	...
	if (argc != 13)
	{
		cout << "Application startup error: Bad number of input parameters. "
				<< endl;
		cout
				<< "Format : ./testmain filePath               fileType imgCount imgType expTime rm  top left width heigh fr dcm"
				<< endl;
		cout
				<< "Example: ./testmain /observations/img/raw/  FITS     100       LF     0.001  1x1  0    0  1000  1000   1  1 "
				<< endl;
		cout
				<< "Example: ./testmain /observations/img/raw/  SBIG      5        DF     15.5   2x2  0    0   0     0     1  1 "
				<< endl;
		return CE_BAD_PARAMETER;
	}
	...
	cout << "Input parameters      : " << "filePath = " << filePath
			<< ", bFitsType = " << bFitsType << ", numOfImages = " << numOfImages
			<< ", bLightFrame = " << bLightFrame << ", exposure time = " << expTime
			<< ", readout mode = " << rm << ", top = " << top << ", left = " << left
			<< ", width = " << width << ", height = " << height << ", bFastReadout = "
			<< bFastReadout << ", bDualChannelMode = " << bDualChannelMode << endl;
	...
	cout << "-----------------------------------------------" << endl;
	cout
			<< "Application name                   : testmain utilizing sbigudrv C library"
			<< endl;
	cout << "Camera frame rates measurement     : NO ERROR" << endl;
	cout << "Average of                         : " << numOfImages
			<< " frames saved to disk." << endl;
		...
		unsigned short height, unsigned short width)
{
	...
	{
		err = (PAR_ERROR) SBIGUnivDrvCommand(CC_READOUT_LINE, &rlp,
				pImg->GetImagePointer() + (long) i * width);
		if (err != CE_NO_ERROR)
		{
...
PAR_ERROR takeImage(int argc, char *argv[])
{
	...
	do
	{
		...
		// start exposure
		err = startExposure(bDualChannelMode, bFastReadout, ccd, expTime,
				shutterCommand, (READOUT_BINNING_MODE) rm, top, left, height, width);
		if (err != CE_NO_ERROR)
		{
			break;
		}
		...
		// start readout
		err = startReadout(ccd, (READOUT_BINNING_MODE) rm, top, left, height,
				width);
		if (err != CE_NO_ERROR)
		{
			break;
		...
		// ccd readout
		err = ccdReadout(pImg, bDualChannelMode, ccd, (READOUT_BINNING_MODE) rm,
		...
		dt = (((t2.tv_sec - t1.tv_sec) * 1000000.0) + (t2.tv_usec - t1.tv_usec))
				/ 1000000.0;
		fprintf(stderr, "exposureTime               : %.3f [sec]\n", dt);

		dt = (((t4.tv_sec - t3.tv_sec) * 1000000.0) + (t4.tv_usec - t3.tv_usec))
				/ 1000000.0;
		fprintf(stderr, "readoutTime                : %.3f [sec]\n", dt);

		dt = (((t4.tv_sec - t1.tv_sec) * 1000000.0) + (t4.tv_usec - t1.tv_usec))
				/ 1000000.0;
...
PAR_ERROR checkQueryTemperatureStatus()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		PAR_ERROR err = (PAR_ERROR) SBIGUnivDrvCommand(CC_QUERY_TEMPERATURE_STATUS,
				&qtsp, &qtsr2);
		fprintf(stderr, "===================================\n");
